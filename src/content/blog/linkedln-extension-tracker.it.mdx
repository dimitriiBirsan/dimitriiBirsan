...
La riga `chrome-extension://invalid/` è la spia. È una sonda deliberata verso un'estensione garantita inesistente — un colpo di calibrazione. Tutto ciò che segue è lo stesso meccanismo puntato verso ID di estensioni reali. (Vedere screenshot: lo stack di chiamate della richiesta fallita è `window.fetch` → una funzione anonima → `await in (anonymous)` → `d`. Tenetevi questo `d`; è il protagonista.)
...
Non vi dirò cosa *significa*. Mi limito a notare che questa è la stessa azienda multata di €310 milioni dall'Irish DPC per la liceità, correttezza e trasparenza del trattamento dei dati dei membri per l'analisi comportamentale e la pubblicità mirata, a seguito di un esposto risalente al 2018 presentato dall'organizzazione non profit francese La Quadrature du Net. Collegare questi due fatti è lasciato, nella migliore tradizione di LinkedIn, come un'opportunità di thought leadership per il lettore.

Esistono contromisure — profili browser separati, container dedicati, configurazioni di estensioni più rigide — ma questo è un altro discorso. Per ora: la prossima volta che LinkedIn vi chiede di "aggiungere una competenza al vostro profilo," ricordate che ha già inventariato quelle nel vostro browser. *Onorato e commosso di condividere questo aggiornamento.*
